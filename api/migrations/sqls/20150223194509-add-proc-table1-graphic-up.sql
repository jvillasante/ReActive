...
AS
$BODY$
  SELECT 'Participación Reunión' AS col, theDate, ROUND((100.0 * yes_count) / total_count, 2) AS percent
    FROM (
      SELECT
  ...
  SELECT 'Registros e Indicadores' AS col, theDate, ROUND((100.0 * yes_count) / total_count, 2) AS percent
    FROM (
      SELECT
        updated_at::date AS theDate,
        sum(CASE WHEN lower(value)='si' THEN 1 ELSE 0 END) AS yes_count,
  ...
  SELECT 'Planificación Semana Anterior' AS col, theDate, ROUND((100.0 * yes_count) / total_count, 2) AS percent
    FROM (
      SELECT
        updated_at::date AS theDate,
        sum(CASE WHEN lower(value)='si' THEN 1 ELSE 0 END) AS yes_count,
  ...
  SELECT 'Plan Intermedio' AS col, theDate, ROUND((100.0 * yes_count) / total_count, 2) AS percent
    FROM (
      SELECT
        updated_at::date AS theDate,
        sum(CASE WHEN lower(value)='si' THEN 1 ELSE 0 END) AS yes_count,
  ...
  SELECT 'Planificación Semanal' AS col, theDate, ROUND((100.0 * yes_count) / total_count, 2) AS percent
    FROM (
      SELECT
        updated_at::date AS theDate,
        sum(CASE WHEN lower(value)='si' THEN 1 ELSE 0 END) AS yes_count,

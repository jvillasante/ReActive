...
UserProvider.prototype.findAll = function(callback) {
  db.connect(this.connStr, function(err, client, done) {
    ...
    client.query("SELECT id, username, email, role, emp, photo FROM users", function(err, result) {
      if (err) {
        done();
...
UserProvider.prototype.findByUsername = function(username, callback) {
  db.connect(this.connStr, function(err, client, done) {
    ...
    client.query("SELECT id, username, email, password, role, emp, photo FROM users WHERE username=$1 LIMIT 1", [username], function(err, result) {
      if (err) {
        done();
        return callback(Err("db query error", { code: 1002, description: err.message, errors: []}));
      }
...
UserProvider.prototype.findById = function(id, callback) {
  db.connect(this.connStr, function(err, client, done) {
    ...
    client.query("SELECT id, username, email, role, emp, photo FROM users WHERE id=$1 LIMIT 1", [id], function(err, result) {
      if (err) {
        done();
        return callback(Err("db query error", { code: 1002, description: err.message, errors: []}));
      }
  ...
  this.validate(user, function(err, user) {
    ...
    bcrypt.hash(user.password, function(err, hash) {
      ...
      db.connect(self.connStr, function(err, client, done) {
        ...
        if (user.id) {
          client.query("INSERT INTO users(id, username, email, password, role, emp, photo) VALUES($1, $2, $3, $4, $5, $6, $7) RETURNING id",
            [user.id, user.username, user.email, hash, user.role, user.emp, user.photo], function(err, result) {
            if (err) {
              done();
              return callback(Err("db query error", { code: 1002, description: err.message, errors: []}));
            }
          ...
          client.query("INSERT INTO users(username, email, password, role, emp, photo) VALUES($1, $2, $3, $4, $5, $6) RETURNING id",
            [user.username, user.email, hash, user.role, user.emp, user.photo], function(err, result) {
            if (err) {
              done();
              return callback(Err("db query error", { code: 1002, description: err.message, errors: []}));
            }
